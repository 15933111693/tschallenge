...
declare function defineStore<
  D extends { [key: PropertyKey]: unknown },
  G extends { [key: string]: Function },
  A extends { [key: string]: Function },
  GG = { [K in keyof G]: G[K] extends () => infer RT ? RT : never }
>(store: {
  id: string;
  state: () => D;
  getters: G & ThisType<Readonly<D> & GG>;
  actions: A & ThisType<D & A & Readonly<GG>>;
}): D & GG & A;
